<!--
WP-005 v2 リライト版｜会計・確定申告ソフト比較（弥生 vs マネーフォワード）
狙い: 感情強め・購買心理設計。確定申告の"あの憂うつ"で共感→ソフトで解放→料金/機能の正確な比較（表は事実厳守）→正直なデメリット→自然なCTA。
文字数目標: 5,500-6,500字（比較型の適正レンジ）
料金は2026年8月時点・公開情報ベース。弥生=税込/MF=税抜。数字はv1（確認済み）を厳守。虚偽体験談・誇大表現なし（§4）。
affiliate: A8(やよいの青色申告オンライン, MFクラウド確定申告) or もしも(MF) / link status: pending
-->
