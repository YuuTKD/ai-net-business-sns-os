...
- **ステータス**: TODO（予定・2026-08-12実施予定とゆうさんより明言）
- **ブランチ**: N/A（WordPress.com/n8n側の課金操作、リポジトリ変更なし）
- **PR**: N/A
- **期限**: 2026-08-12（水）
- **備考**: 現在は無料プラン（treecosme.home.blog、独自ドメイン不可・SEOプラグイン導入不可・広告表示あり）。2026-08-11深夜、Claude CodeがWordPress管理画面のプラン比較ページまで画面操作で提示済み（ビジネスプラン月$25を推奨：Yoast等SEOプラグイン導入にはプレミアム以下では不可、ビジネスプラン以上が必須と判明）。決済はゆうさん本人が実施（CLAUDE.md禁止事項：カード情報入力はAI不可）。n8nも同日に有料化予定（Credential登録・ワークフロー本格稼働に必要）。アップグレード後のフォローアップ：独自ドメイン設定・Yoast SEO導入・広告非表示・Search Console連携の本格運用・n8n Credential登録（Gumroad/Threads APIトークン）。詳細は`operations/REVENUE_ACCELERATION_PLAN_20260811.md`参照。
