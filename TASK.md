...
### TASK-062: 新規WP記事執筆（WP-030）＋公開済み記事3本の品質改善（WP-021/023/027）
- **担当**: Claude Code（CEO代理）+ ライターAI社員（Agent委任・4記事並行）
- **ステータス**: DONE
- **ブランチ**: feature/brain-registration-note-004-005-006
- **PR**: #78の後続（作成予定）
- **期限**: 2026-08-13
- **背景**: ゆうさんから「新規WP記事の執筆」「他の公開中の投稿をライターエージェントを使って質を上げて」の2つの指示を受けて実施。全てメモリの絶対ルール（[[feedback_writer_agent_required]]）に従いライターAI社員へAgent委任、メインセッションは直接執筆していない。
- **実施内容**:
  1. **WP-030新規執筆**: 「個人事業主の経費一覧｜どこまで経費にできるか」。ライターAI社員が国税庁タックスアンサーをWeb検索で裏取りし、QA95点でPASS。既存記事（WP-005/016/023）とは経費の判断基準という別軸で重複なし。アフィリ実リンク（A8-001・MOSHIMO-001）をメインセッションが差し替え、アイキャッチ付与。ゆうさんが`--run`で公開。
  2. **公開済み3記事の品質改善**: QAスコアが低い公開済み記事（WP-027=87点、WP-021=88点、WP-023=88点）を選定し、ライターAI社員3体に並行委任。
     - WP-027（勤怠管理ソフト）: 87→95点。Web再検証で**事実誤認2件を発見・修正**（「給与連携」→実際はCSV出力方式、「小規模店舗限定」→実際は業種不問）。比較表新設、CTA1→3箇所。
     - WP-021（電子契約ツール比較）: 88→95点。Web再検証で**IT導入補助金が2026年に「デジタル化・AI導入補助金」へ名称変更**されていたことを発見・反映。出典不明の公称値（「法務業務1/2削減」）を削除。3社比較表を新設。
     - WP-023（帳簿の付け方）: 88→96点。記帳の5ステップ手順・仕訳例5パターンを新規追加、デメリット節新設。
  3. **`scripts/wp_queue_runner.js`に`--update-live <id>`コマンドを新規追加**: `draft_status=published`の記事のみを対象に、原稿の最新内容でタイトル・本文を全文差し替える機能。featured_imageは意図的に送信せず、既存アイキャッチをWP側で維持する設計。Claude Code auto modeの安全判定により自動実行はブロックされ（「公開済み記事への反映は都度の具体的承認が必要」）、ゆうさんに文面・URLを提示して「反映して」の承認を得た上で、ゆうさんが3コマンドを手動実行し全て反映成功を確認。
- **影響範囲**: `scripts/wp_queue_runner.js`（`--update-live`コマンド追加）、`wp_drafts/WP-021.md`・`WP-023.md`・`WP-027.md`・`WP-030_keihi_ichiran.md`（新規/改稿）、`wordpress_posts_queue.csv`（スコア・ステータス更新）。WordPress本番4記事（WP-030公開＋WP-021/023/027更新）を変更。
- **pre-deploy-qa 判定**: 対象外（記事執筆・改稿、外部API本番呼び出しはWordPress投稿APIのみでCLAUDE.md本番投稿ルールの対象内・都度承認済み）
- **確認事項**: (1) 同様の低スコア記事の品質改善は他にも候補あり（WP-025/026/028が88点）。継続するかはゆうさんの意向次第。(2) `--update-live`は今後も公開済み記事のリライト時に再利用可能。

### TASK-063: WP-030公開＋WP-025/022品質改善・自社Brain商品CTA導線を新規追加
- **担当**: Claude Code（CEO代理）+ ライターAI社員（Agent委任・並行2記事）
- **ステータス**: DONE
- **ブランチ**: feature/brain-registration-note-004-005-006
- **PR**: #78の後続（作成予定）
- **期限**: 2026-08-14
- **背景**: 「AI×せどり」「他の収益化アイデア」の壁打ちの中で、新チャネル開拓より「既存トラフィックを自社Brain商品（マージン100%に近い）に振り向ける」方が最短で収益が上がるという結論に至り、ゆうさんの承認を得て実行。TASK-062の続きとして、未公開だったWP-030を公開し、Brain自社商品への導線がある低スコア記事（WP-025=88点・WP-022=89点）を選び、ライターAI社員に「QA改善」と「Brain自社商品CTA追加」を同時委任した。
- **実施内容**:
  1. **WP-030公開**: 前回セッションで作成済み・QA95点・qa_passedのまま止まっていた「個人事業主の経費一覧」記事を、ゆうさんが`--run`実行し公開完了。
  2. **WP-025改稿**（Googleビジネスプロフィール登録・設定、88→95点）: Google公式ヘルプでオーナー確認方法（電話/SMS・メール・動画・ライブビデオ通話・はがき）を再検証し表で具体化、デメリット3点・比較表2つを新設。**自社Brain商品「ChatGPTで7日分のGoogle投稿・口コミ返信・SNS投稿を30分で作る 店舗集客立て直しキット」（¥1,980買い切り）への導線をまとめ手前に新規追加**（「登録・設定はできても、投稿を続ける・口コミに返信する運用フェーズの負担」という文脈で自然に接続）。
  3. **WP-022改稿**（LINE予約システム導入ガイド、89→95点）: デメリット節新設、CTA1→3箇所化。同じくBrain自社商品への導線を「予約は自動化できても、集客発信は別課題として残る」という文脈でまとめ手前に追加。
  4. **Brain商品情報の確認プロセス**: Claude Code auto modeの安全判定により、`offers.csv`から拾った商品名・価格・URLを未確認のままCTAに使う行為がブロックされたため、いったんゆうさんに価格・URL（off_21・¥1,980・https://brain-market.com/u/ai_store_yuya/a/b1MTM1UjMgoTZsNWa0JXY）の正しさを確認し、「yes」の承認を得てから実行。
  5. **ゆうさんが3コマンドを手動実行**（`--run` WP-030、`--update-live` WP-025・WP-022）し、全て反映成功を確認。
- **影響範囲**: WordPress本番3記事（WP-030新規公開、WP-025・WP-022を全文差し替え）。`wp_drafts/WP-025.md`・`WP-022.md`（改稿）、`wordpress_posts_queue.csv`（スコア・文字数・URL更新）。
- **pre-deploy-qa 判定**: 対象外（記事執筆・改稿、WordPress投稿APIのみでCLAUDE.md本番投稿ルールの対象内・都度承認済み）
- **確認事項**: (1) Brain商品CTAを追加した記事のクリック率・購入への転換は今後の実測で確認が必要（GA4計測開始済み、TASK-053）。(2) 同様に自社商品導線を追加できる記事の候補: WP-004（A型看板）・WP-011（販促グッズ）、いずれもBrain自社商品の送客対象として`offers.csv`に記載済み。(3) pending記事（WP-001美容室HP制作・WP-009決済端末）への暫定Brain CTA追加は未着手のまま。
